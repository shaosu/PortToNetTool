...
namespace ZLGCAN
{
    ...
    public class Method
    {
        private const string CanDllPath = "Config\\ZL\\zlgcan.dll";

        [DllImport(CanDllPath, CallingConvention = CallingConvention.StdCall)]
        ...
        /// <param name="eff">
        /// <part>帧格式</part>
        /// <part>1:extend frame 0:standard frame</part></param>
        /// <param name="rtr">远程帧</param>
        /// <param name="err"></param>
        /// <returns></returns>
        public static uint MakeCanId(uint id, int eff, int rtr, int err)
        {
            id = id & 0x1FFFFFFF;
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public static (byte ExternFlag, byte RemoteFlag, byte err) GetZCanIdInfo(uint id)
        {
            var ueff = (id & (1 << 31)) > 0 ? (byte)1 : (byte)0;
            var urtr = (id & (1 << 30)) > 0 ? (byte)1 : (byte)0;
            var uerr = (id & (1 << 29)) > 0 ? (byte)1 : (byte)0;
            return (ueff, urtr, uerr);
        }
        public static uint GetZCanIdOnlyID(uint id)
        {
